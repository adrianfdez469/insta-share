...
export class FilesController {
  ...
  async handleFileCompressedUpload(data: {userId: string, name: string, size: number, originalname: string, savePath: string}) {
    ...
    await this.filesService.saveFile(data.originalname, 'UPLOADED', data.size, data.name, data.userId );
  }
  ...
  async handleFileCompressing(data: {uniquename: string}) {
    ...
    await this.filesService.updateByUrl(data.uniquename, {status: "COMPRESSING"});
  }
  ...
  async handleFileCompressedSaved(data: {userId: string, old_uniquename: string, new_compressed_name: string, new_size: number}) {
    ...
    await this.filesService.updateByUrl(data.old_uniquename as string, {status: 'COMPRESSED', zipped_size: Number(data.new_size), url: data.new_compressed_name as string});
  }
}